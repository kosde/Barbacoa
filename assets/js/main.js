(function ($) {
    ...
    $(".qtybutton").on("click", function () {
        ...
        if($button.prev().attr("id"))
        {
            var idButton    = $button.prev().attr("id");
        }
        if($button.next().attr("id"))
        {
            var idButton    = $button.next().attr("id");
        }
        if(idButton == "33")
        {
            if ($button.text() === "+") {
    ...
    var CartPlusMinus = $('.cart-plus-minus2');
    ...
    $(".qtybutton2").on("click", function () {
        ...
        {
            var idButton    = $button.prev().attr("id");
            var idproduct =  $button.prev().attr("title");
        }
        if($button.next().attr("id"))
        {
            var idButton    = $button.next().attr("id");
            var idproduct =  $button.next().attr("title");
        }
        if ($button.text() === "+") {
        ...
        var precio = document.getElementById("precio_unitario_"+idButton).innerHTML;
        precio =  precio.replace(/[^0-9\.]+/g,"");
        console.log(precio);
        var priceT = precio * newVal
        update(idproduct,newVal,priceT);
        document.getElementById("id"+idButton).innerHTML = "$ "+ priceT + ".00";    
        $button.parent().find("input").val(newVal);
    });
